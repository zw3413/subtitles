...
func GetTodayVisitedSubtitlesByUser(email, userUUID string, hours string) []string {
	...
	subtitle_uuids := make([]string, 0)
	for _, d := range data {
		subtitle_uuids = append(subtitle_uuids, d["subtitle_uuid"].(string))
	}
	return subtitle_uuids
}

func SaveSubtitleLog(email, userUUID, subtitleUUID, limitType string) error {
	sql := `
		insert into subtitle_log (email, user_uuid, subtitle_uuid, type)
		values ($1, $2, $3, $4)
	`
	_, err := utils.GetAllData(sql, email, userUUID, subtitleUUID, limitType)
	if err != nil {
		return err
	} else {
		return nil
